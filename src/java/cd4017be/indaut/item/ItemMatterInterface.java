	...
	public void onPlayerCommand(ItemStack item, EntityPlayer player, PacketBuffer dis) {
		if (player.openContainer instanceof TileContainer && ((TileContainer)player.openContainer).data instanceof GuiData) {
			...
			if (cmd == 0) {
				...
				if (am <= 0 || dst < 0 || dst >= inv.inv.mainInventory.size()) return;
				ItemStack A = inv.inv.mainInventory.get(inv.tool);
				ItemStack B = inv.inv.mainInventory.get(dst);
				...
				if (inv.tool < 0 || inv.tool >= inv.inv.mainInventory.size()) inv.tool = inv.inv.currentItem;
			} else if (cmd == 2) {
