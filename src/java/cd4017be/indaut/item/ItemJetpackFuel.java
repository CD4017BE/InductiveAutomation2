...
public class ItemJetpackFuel extends DefaultItem {
	...
	public void addInformation(ItemStack item, EntityPlayer player, List<String> list, boolean b) {
		if (item.hasTagCompound()) {
			list.add(String.format("H2= %s / %d %s", TooltipUtil.formatNumber(item.getTagCompound().getFloat("H2") / 1000F, 3), Config.tankCap[2] / 500, TooltipUtil.getFluidUnit()));
			list.add(String.format("O2= %s / %d %s", TooltipUtil.formatNumber(item.getTagCompound().getFloat("O2") / 1000F, 3), Config.tankCap[2] / 1000, TooltipUtil.getFluidUnit()));
		}
	...
	public static float useFuel(float n, InventoryPlayer inv, int slot) {
		ItemStack item = inv.mainInventory.get(slot);
		if (item == null || item.getItem() == null || !item.hasTagCompound()) return 0;
