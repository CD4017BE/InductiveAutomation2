...
public class ItemMachineSynchronizer extends DefaultItem {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing s, float X, float Y, float Z) {
		...
		if (player.isSneaking()) {
			...
			stack = new ItemStack(this, stack.getCount(), m);
			...
			player.sendMessage(new TextComponentString("Linked to machine"));
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		if (!world.isRemote) player.sendMessage(new TextComponentString("Link cleared"));
		return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, new ItemStack(this, item.getCount(), 0));
	}
