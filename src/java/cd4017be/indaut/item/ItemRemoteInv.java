...
public class ItemRemoteInv extends DefaultItem implements IGuiItem, IItemInventory {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing s, float X, float Y, float Z) {
		...
		if (!world.isRemote && player.isSneaking()) {
			...
			if (acc == null) {
				player.sendMessage(new TextComponentString("Block has no inventory!"));
			...
			player.sendMessage(new TextComponentString("Block inventory linked"));
			return EnumActionResult.SUCCESS;
		}
	...
	public static final IFilter<ItemStack, IItemHandler> notMe = new IFilter<ItemStack, IItemHandler>(){
		...
		public int insertAmount(ItemStack obj, IItemHandler inv) {
			return obj == null || obj.getItem() instanceof ItemRemoteInv || obj.getItem() instanceof ItemFilteredSubInventory ? 0 : obj.getCount();
		}
		@Override
