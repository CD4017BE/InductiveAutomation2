...
import cd4017be.indaut.item.ItemFluidUpgrade;
...
public class FluidPipe extends BaseTileEntity implements ITilePlaceHarvest, INeighborAwareTile, IInteractiveTile, ITickable, IModularTile {
	...
	private final LinkedTank tankcap = new LinkedTank(Config.tankCap[0], this::getFluid, this::setFluid);
	...
	public boolean onActivated(EntityPlayer player, EnumHand hand, ItemStack item, EnumFacing dir, float X, float Y, float Z) {
		...
		if (player.isSneaking() && item.getCount() == 0) {
			...
			markUpdate();
			return true;
		} else if (filter == null && canF && item.getItem() instanceof ItemFluidUpgrade && item.getTagCompound() != null) {
