...
import cd4017be.indaut.AreaConfig;
...
public class SecuritySys extends AutomatedTile implements IGuiData {
	...
	public void update() 
	{
		...
		boolean rst = world.isBlockPowered(getPos());
	...
	public boolean onActivated(EntityPlayer player, EnumHand hand, ItemStack item, EnumFacing s, float X, float Y, float Z) 
	{
		if (!prot.isPlayerOwner(player.getName())){
			if (!world.isRemote) player.sendMessage(new TextComponentString("You are not given the necessary rights to use this!"));
			return true;
		} else if (player.isSneaking() && player.getHeldItemMainhand() == null) {
			if (!itemStorage.isEmpty()) {
				//TODO drop confiscated items;
			}
			this.getBlockType().dropBlockAsItem(world, getPos(), world.getBlockState(pos), 0);
	...
	public void onPlaced(EntityLivingBase entity, ItemStack item)  
	{
		...
		if ((AreaProtect.permissions > 0 && AreaProtect.chunkloadPerm > 0) || !(world instanceof WorldServer)) return;
		PlayerList manager = ((WorldServer)world).getMinecraftServer().getPlayerList();
		...
		if (perm == 0) return;
		if (perm == 3) world.setBlockToAir(getPos());
		if (entity instanceof EntityPlayer) ((EntityPlayer)entity).sendMessage(new TextComponentString(perm == 3 ? "You are not allowed to use this device on this server !" : perm == 2 ? "Chunk loading functionality of this device disabled for you !" : "Chunk protection functionality of this device disabled for you !"));
	}
