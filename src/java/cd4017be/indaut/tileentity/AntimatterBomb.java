...
public class AntimatterBomb extends ModTileEntity implements ITickable
{
	...
	public void update() 
	{
		...
		if (state == 1 && timer == 0) {
			...
			if (explosion == null) {
				world.setBlockState(pos, Objects.antimatterBombE.getDefaultState());
				if (loot != null) world.setTileEntity(pos, loot);
				return;
			}
	...
	private boolean isProtected()
	{
		...
		return !AreaProtect.operationAllowed(new GameProfile(new UUID(0, 0), "#antimatterBomb"), world, x0, x1, z0, z1);
	}
	
	@Override
	public void onNeighborBlockChange(Block b, BlockPos src) 
	{
		if (state == 0 && world.isBlockPowered(pos)) {
			state = 1;
			timer = 200;
		}
	}
	...
	private class AntimatterExplosion {
			...
			for (j = 0; j < l; j++)
				for (i = 0; i < l; i++) {
					...
					if (state.getBlock().isAir(state, world, pos)) {
						...
						r = state.getBlock().getExplosionResistance(world, pos, null, null);
		...
		private void damageEntitys(int size) {
			...
			List<Entity> list = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos).expandXyz(size));
		...
		private void destroyBlocks() {
			...
			for (Obj2<BlockPos, Boolean> next : destroyedBlocks) {
				...
				if (state.getBlock() == Objects.antimatterBombF) {
					...
					loot.addItems(block.getDrops(world, pos, state, 0));
				}
				else if (next.objB) loot.addItems(block.getDrops(world, pos, state, 0));
				else {
					FluidStack fluid = null;
					if (block instanceof IFluidBlock) fluid = ((IFluidBlock)block).drain(world, pos, false);
				...
				MovedBlock.setBlock(world, pos, Blocks.AIR.getDefaultState(), null);
			}
			destroyedBlocks.clear();
		}

		private void doGammaDamage(float energy) {
			List<Entity> list = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos).expandXyz(maxSize));
			for (Entity entity : list) {
				if (entity instanceof EntityLivingBase) {
					if (world.rayTraceBlocks(new Vec3d(posX, posY, posZ), new Vec3d(entity.posX, entity.posY, entity.posZ), true) == null) {
						...
						entity.attackEntityFrom(DamageSource.IN_FIRE, (int)(d * 2.5));
