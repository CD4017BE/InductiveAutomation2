...
import cd4017be.indaut.item.ItemTeleporterCoords;
...
public class Teleporter extends AutomatedTile implements IOperatingArea, Environment, IGuiData, IAccessHandler, ISlotClickHandler {
	...
	public void update() 
	{
		...
		if ((mode & 1) == 1) {
			boolean r = world.isBlockIndirectlyGettingPowered(pos) > 0;
	...
	private void initTeleportation()
	{
		...
		if (!AreaProtect.operationAllowed(lastUser, world, area[0] >> 4, (area[3] + 15) >> 4, area[2] >> 4, (area[5] + 15) >> 4) ||
	...
	private void teleport()
	{
		...
		if (!world.isAreaLoaded(pos0, pos0.add(size)))
		...
		if (!world.isAreaLoaded(pos1, pos1.add(size)))
			for (int cx = target[0] >> 4; cx <= (target[0] + pos[3]) >> 4; cx++)
				for (int cz = target[2] >> 4; cz <= (target[2] + pos[5]) >> 4; cz++)
					world.getChunkProvider().provideChunk(cx, cz);
		List<Entity> e0 = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos[0], pos[1], pos[2], pos[0] + pos[3], pos[1] + pos[4], pos[2] + pos[5]));
		...
		for (Entity e : e1) MovedBlock.moveEntity(e, world.provider.getDimension(), e.posX - dx, e.posY - dy, e.posZ - dz);
		...
		for (int x = ox ? 0 : pos[3] - 1; ox ? x < pos[3] : x >= 0; x += ox ? 1 : -1)
			for (int y = oy ? 0 : pos[4] - 1; oy ? y < pos[4] : y >= 0; y += oy ? 1 : -1)
				for (int z = oz ? 0 : pos[5] - 1; oz ? z < pos[5] : z >= 0; z += oz ? 1 : -1) {
					...
					MovedBlock b0 = MovedBlock.get(world, pos0);
	...
	protected void customPlayerCommand(byte cmd, PacketBuffer dis, EntityPlayerMP player) throws IOException 
	{
		...
		if (cmd == 0)
		{
			...
			if (b == 3)
			{
				...
				if (inventory.items[0] != null && (inventory.items[0].getItem() instanceof ItemTeleporterCoords || inventory.items[0].getItem() == Items.PAPER))
				{
					...
					inventory.items[0] = new ItemStack(Objects.teleporterCoords, inventory.items[0].getCount());
		...
		{
			inventory.items[0].getTagCompound().setString("name", dis.readString(64));
		}
		mode &= ~8;
	}
