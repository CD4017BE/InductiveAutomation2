		...
		public ItemStack insertItem(int slot, ItemStack item, boolean sim) {
			...
			for (int i = 0; i < 4; i++, c >>= 2)
				if ((c & 3) == 1 && (filt = filter[i]) != null) {
					...
					if ((filt.mode & 32) != 0 && p >= 0) n = Math.min(item.getMaxStackSize(), filt.list[p].getCount());
					...
					if (item.getCount() <= n) {
						if (!sim)
							...
							else stack.grow(item.getCount());
						return null;
					} else if (!sim)
						if (stack == null) inventory.items[i] = item.splitStack(n);
						else {stack.grow(n); item.shrink(n);}
					else item.shrink(n);
					if (match && (filt.mode & 2) != 0) return item;
				}
			if ((c & 3) == 1) {
				...
				if (stack == null) {
					...
					int m = stack.getMaxStackSize() - stack.getCount();
					if (item.getCount() <= m) {
						if (!sim) stack.grow(item.getCount());
						return null;
					}
					item.shrink(m);
					if (!sim) stack.grow(m);
