...
import cd4017be.indaut.item.ItemBuilderTexture;
...
public class Builder extends AutomatedTile implements IOperatingArea, Environment, IGuiData, IAccessHandler, ISlotClickHandler, IQuickRedstoneHandler {
	...
	private boolean placeBlock(int dir)
	{
		...
		if (!world.isBlockLoaded(new BlockPos(px, py, pz))) return false;
		int i = 0, j = 0;
	...
	private boolean setBlock(BlockPos pos, ItemStack item)
	{
		...
		if (item.getItem() instanceof ItemBlock) {
			...
			if (comp == 0) {
				...
				if (b != null && !b.isReplaceable(world, pos)) return true;
			}
			...
			if (storage >= Energy && (stack = remove(inv.inventory[comp], inv.useDamage(comp))) != null) {
				...
				stack = inv.doPlacement(world, player, pos, comp, stack);
				storage -= Energy;
				if (stack != null)
		...
		Block b = world.getBlockState(pos).getBlock();
		if (!b.isReplaceable(world, pos)) return true;
		if (storage >= Energy && remove(item, true) != null)
		{
			world.setBlockState(pos, id.getStateFromMeta(m), 0x3);
			storage -= Energy;
			return true;
		} else
		return false;
	...
	private static class BuildTask {
		...
		public boolean execute(Builder builder)
		{
			...
			if (!builder.world.isBlockLoaded(pos)) return false;
			Block block = builder.world.getBlockState(pos).getBlock();
			boolean can = block == null || block.isReplaceable(builder.world, pos);
	...
	public void onNeighborBlockChange(Block b, BlockPos src) {
		for (EnumFacing s : EnumFacing.VALUES) {
			int rs = world.getRedstonePower(pos.offset(s), s);
			if ((rs & 0x7c000000) != 0) {
