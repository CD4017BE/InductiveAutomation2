...
public class GraviCond extends AutomatedTile implements IGuiData, ISlotClickHandler {
	...
	public void update() 
	{
		...
		if (inventory.items[0] != null) {
			if (inventory.items[0].getItem() instanceof ItemBlock) add += inventory.items[0].getCount() * blockWeight;
			else add += inventory.items[0].getCount() * itemWeight;
		...
		if (inventory.items[1] != null) {
			...
			if (rcp != null) {
				need = rcp.matter;
				if (matter >= need && (inventory.items[2] == null || (Utils.itemsEqual(rcp.output, inventory.items[2]) && rcp.output.getCount() + inventory.items[2].getCount() <= rcp.output.getMaxStackSize()))) {
					matter -= need;
					inventory.items[1].shrink(rcp.input.getCount());
					if (inventory.items[1].getCount() <= 0) inventory.items[1] = null;
					if (inventory.items[2] == null) inventory.items[2] = rcp.output.copy();
					else inventory.items[2].grow(rcp.output.getCount());
				}
			}
		}
	}
