...
public class AlgaePool extends AutomatedTile implements IGuiData, ISlotClickHandler {
	...
	public void update() 
	{
		...
		if (inventory.items[0] != null && inventory.items[0].isItemEqual(BlockItemRegistry.stack("m.LCAlgae", 1))) {
			algae += inventory.items[0].getCount() * 1000;
			inventory.items[0] = new ItemStack(Items.GLASS_BOTTLE, inventory.items[0].getCount());
		}
		if (inventory.items[1] != null && inventory.items[1].isItemEqual(new ItemStack(Items.GLASS_BOTTLE)) && algae >= inventory.items[1].getCount() * 1000) {
			algae -= inventory.items[1].getCount() * 1000;
			inventory.items[1] = BlockItemRegistry.stack("m.LCAlgae", inventory.items[1].getCount());
		}
		...
		if (n != null && nutrient + n[0] <= Config.tankCap[1] && tanks.getAmount(0) >= n[0])
		{
			...
			nutrient += n[0];
			algae += n[1];
			inventory.items[4].grow(-1);
			if (inventory.items[4].getCount() <= 0) inventory.items[4] = null;
		}
	}
	...
	private void updateMultiblock(BlockPos p, List<BlockPos> list)
	{
		for (int i = 0; i < 6 && list.size() < 16; i++)
		{
			...
			if (world.getBlockState(pos).getBlock() != Objects.pool) continue;
			boolean b = true;
			...
			{
				list.add(pos);
				midBrightness += world.getLightFor(EnumSkyBlock.SKY, pos.up());
				updateMultiblock(pos, list);
			}
		}
	}
	...
	{
		float l = midBrightness - world.getSkylightSubtracted();
		if (l < 0) l = 0;
