...
import cd4017be.indaut.shaft.ShaftComponent;
import cd4017be.indaut.shaft.ShaftPhysics.IKineticComp;
...
public class ElectricCoil extends AutomatedTile implements IKineticComp, IGuiData {
	...
	public void update() {
		...
		Uc = energy.Ucap;
		if (link != null) return;
		if ((rstCfg & 1) == 0 ^ ((rstCfg & 2) != 0 && world.getStrongPower(pos) > 0)) {
			power = 0;
			return;
		} 
		...
		ElectricCoil ec;
		if (te != null && te instanceof ElectricCoil && (ec = (ElectricCoil)te).getOrientation() == (o^1) && ec.energy.Umax != 0 && ((ec.rstCfg & 1) != 0 ^ ((ec.rstCfg & 2) != 0 && ec.world.getStrongPower(ec.pos) > 0))) {
			if (o % 2 != 0) return;
	...
	public float estimatedForce(float ds) {
		if ((rstCfg & 1) == 0 ^ ((rstCfg & 2) != 0 && world.getStrongPower(pos) > 0)) return F = 0;
