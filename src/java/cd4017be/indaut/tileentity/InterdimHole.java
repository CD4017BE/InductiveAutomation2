...
public class InterdimHole extends ModTileEntity implements ITickable {
	...
	public void onPlaced(EntityLivingBase entity, ItemStack item) 
	{
		...
		if (item.getItemDamage() == 0) {
			...
			drop.getTagCompound().setInteger("ld", world.provider.getDimension());
			EntityItem eitem = new EntityItem(world, entity.posX, entity.posY, entity.posZ, drop);
			world.spawnEntity(eitem);
			if (entity instanceof EntityPlayer) ((EntityPlayer)entity).sendMessage(new TextComponentString("The droped Wormhole will link to this"));
			...
			link(true);
			if (linkTile != null && entity instanceof EntityPlayer) ((EntityPlayer)entity).sendMessage(new TextComponentString(String.format("Link found in dimension %d at position %d , %d , %d", linkD, linkX, linkY, linkZ)));
			else if (entity instanceof EntityPlayer) ((EntityPlayer)entity).sendMessage(new TextComponentString("Error: Link not Found!"));
		}
		this.init();
	}
	...
	public boolean onActivated(EntityPlayer player, EnumHand hand, ItemStack stack, EnumFacing s, float X, float Y, float Z) 
	{
		...
		if (player.isSneaking() && stack == null && linkTile != null && !linkTile.isInvalid() && linkTile.linkTile == this) {
			...
			world.spawnEntity(eitem);
			player.sendMessage(new TextComponentString("Both linked Wormholes removed"));
			return true;
		} else if (stack != null && stack.getItem() == Items.PAPER) {
			ItemStack item = new ItemStack(Objects.teleporterCoords, player.getHeldItemMainhand().getCount());
	...
	public int redstoneLevel(int s, boolean str) 
	{
		...
		if (str) return 0;
		return linkTile.world.getRedstonePower(new BlockPos(linkX - dir.getFrontOffsetX(), linkY - dir.getFrontOffsetY(), linkZ - dir.getFrontOffsetZ()), EnumFacing.VALUES[s]);
	}
	...
	public void onEntityCollided(Entity entity) {
		...
		AxisAlignedBB block = state.getCollisionBoundingBox(world, this.pos);
		...
		MovedBlock.moveEntity(entity, linkTile.world.provider.getDimension(), pos.x, pos.y, pos.z);
	}

	protected boolean isAreaClear(AxisAlignedBB area) {
		int x0 = MathHelper.floor(area.minX);
		int x1 = MathHelper.floor(area.maxX + 1.0D);
		int y0 = MathHelper.floor(area.minY);
		int y1 = MathHelper.floor(area.maxY + 1.0D);
		int z0 = MathHelper.floor(area.minZ);
		int z1 = MathHelper.floor(area.maxZ + 1.0D);
		BlockPos pos;
			...
			for (int z = z0; z < z1; ++z)
				if (world.isBlockLoaded(new BlockPos(x, 64, z)))
					for (int y = y0 - 1; y < y1; ++y) {
						...
						block.addCollisionBoxToList(world, pos, area, list, (Entity)null, false);
	...
	public void init() {
		...
		for (int i = 0; i < 6; i++)
		{
			...
			neighbors[i] = world.getTileEntity(pos.offset(dir));
	...
	private void link(boolean force) {
		...
		if (world != null) {
			...
			if (te != null && te instanceof InterdimHole) {
				...
				linkTile.linkD = world.provider.getDimension();
			}
		}
	}
