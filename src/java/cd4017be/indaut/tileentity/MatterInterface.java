...
public class MatterInterface extends AutomatedTile implements IGuiData, IAccessHandler {
	...
	public void update() {
		...
		if ((mode & 3) == 2) {
			...
			for (EnumFacing s : EnumFacing.VALUES)
				rs |= world.getRedstonePower(pos, s);
			rs &= 0xff;
			if (rs != selSlot) {
				selSlot = rs;
				updateOrb();
	...
	public void setSlot(int g, int s, ItemStack item) {
		if(world.isRemote) inventory.items[s] = item;
		else switch(s) {
		case 0:
		...
		case 2:
			...
			if (last == null) return;
			int n = last.getCount() - (item != null ? item.getCount() : 0);
	...
	public int insertAm(int g, int s, ItemStack item, ItemStack insert) {
		...
		case 1: return MatterOrbItemHandler.canInsert(inventory.items[0], insert) ? insert.getCount() : 0;
		default: return 0;
		}
	}
