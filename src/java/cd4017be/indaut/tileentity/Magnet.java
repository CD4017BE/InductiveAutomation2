...
public class Magnet extends ModTileEntity implements ITickable {
	...
	public void update() 
	{
		...
		Iterator<Entity> list = world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos.getX() + 0.5D - rad, pos.getY() + 0.5D - rad, pos.getZ() + 0.5D - rad, pos.getX() + 0.5D + rad, pos.getY() + 0.5D + rad, pos.getZ() + 0.5D + rad)).iterator();
		...
		while (list.hasNext())
		{
			...
			if (entity instanceof EntityItem && entity.isEntityAlive())
			{
				...
				if (item == null) world.removeEntity(entity);
