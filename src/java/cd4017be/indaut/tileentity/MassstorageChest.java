...
public class MassstorageChest extends AutomatedTile implements IGuiData, IAccessHandler {
	...
	public void update() {
		super.update();
		if (!world.isRemote && inventory.items[0] != null) {
			this.addStack();
		}
	}
	...
	private void addStack() {
		...
		for (int i = 1; i < inventory.items.length; i++) {
			...
			if (inventory.items[i] != null && Utils.itemsEqual(inventory.items[i], inventory.items[0])) {
				inventory.items[i].grow(inventory.items[0].getCount());
				if (inventory.items[i].getCount() > StackLimit) {
					inventory.items[0].setCount(inventory.items[i].getCount() - StackLimit);
					inventory.items[i].setCount(StackLimit);
				} else {
	...
	protected void customPlayerCommand(byte cmd, PacketBuffer dis, EntityPlayerMP player) throws IOException {
		if (cmd == 0) {
			...
			for (int i = 0, m = 1; m < 65; m++) {
				...
				else if (arr[i].getCount() <= StackLimit) inventory.items[m] = arr[i++];
	...
	public int insertAm(int g, int s, ItemStack item, ItemStack insert) {
		int m = Math.min((s != 0 ? StackLimit : insert.getMaxStackSize()) - (item != null ? item.getCount() : 0), insert.getCount()); 
