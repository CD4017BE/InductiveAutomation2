...
public class EnergyFurnace extends AutomatedTile implements IGuiData {
	...
	private void putItemStack(int src, int dest)
	{
		...
		{
			int i = inventory.items[dest].getMaxStackSize() - inventory.items[dest].getCount();
			if (inventory.items[src].getCount() < i) i = inventory.items[src].getCount();
			inventory.items[src].shrink(i);
			inventory.items[dest].grow(i);
			if (inventory.items[src].getCount() <= 0) inventory.items[src] = null;
		}
	}
