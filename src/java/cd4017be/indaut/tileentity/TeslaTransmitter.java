...
public class TeslaTransmitter extends ModTileEntity implements ITeslaTransmitter, ITickable, IGuiData
{
	...
	{
		return !this.isInvalid() && world.isBlockLoaded(pos) && world.getTileEntity(pos) == this;
	}
	...
	public double getSqDistance(ITeslaTransmitter te)
	{
		...
		double d;
		if (p[3] == world.provider.getDimension())
		{
			int dx = pos.getX() - p[0];
			int dy = pos.getY() - p[1];
			int dz = pos.getZ() - p[2];
	...
	{
		return new int[]{pos.getX(), pos.getY(), pos.getZ(), world.provider.getDimension()};
	}
