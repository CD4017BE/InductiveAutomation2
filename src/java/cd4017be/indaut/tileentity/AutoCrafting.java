...
public class AutoCrafting extends AutomatedTile implements IGuiData, IAccessHandler
{
	...
	public void update() 
	{
		...
		boolean rst = world.isBlockPowered(pos);
		...
		if (craft && craftOut != null) {
			if (inventory.items[9] == null) {
				...
				craftItem();
				craftChange = true;
			} else if (Utils.itemsEqual(inventory.items[9], craftOut) && inventory.items[9].getCount() <= craftOut.getMaxStackSize() - craftOut.getCount()) {
				inventory.items[9].grow(craftOut.getCount());
				craftItem();
				craftChange = true;
			}
		}
	...
	private ItemStack getCraftingOutput()
	{
		...
		if (inventory.items[i] != null && inventory.items[i].getCount() < am[i]) return null;
		...
		craftChange = false;
		return CraftingManager.getInstance().findMatchingRecipe(icr, world);
	}
	...
	private void putToUnusedSlotOrDrop(ItemStack item)
	{
		...
		if (!used[i]) {
			if (inventory.items[i] == null) {
				...
				if (item.getCount() <= inventory.items[i].getMaxStackSize() - inventory.items[i].getCount()) {
					inventory.items[i].grow(item.getCount());
					return;
				} else
				{
					item.shrink(inventory.items[i].getMaxStackSize() - inventory.items[i].getCount());
					inventory.items[i].setCount(inventory.items[i].getMaxStackSize());
				}
			}
		}
		EntityItem entity = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 1.5D, pos.getZ() + 0.5D, item);
