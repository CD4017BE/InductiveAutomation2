...
import cd4017be.indaut.item.ItemMatterOrb;
...
public class MatterOrb extends ModTileEntity implements IMatterStorage, IInventory
{
	...
	public ItemStack removeItems(int n) 
	{
		...
		if (item.getCount() <= n) return storage.remove(0);
	...
	public void readFromNBT(NBTTagCompound nbt) 
	{
		...
		if (nbt.hasKey("In")) {
			in = new ItemStack(nbt.getCompoundTag("In"));
		} else in = null;
	}
	...
	public NBTTagList writeItems() {
		...
		for (ItemStack item : storage) {
			...
			tag.setInteger("n", item.getCount());
	...
	public ItemStack decrStackSize(int i, int n) 
	{
		...
		if (n >= in.getCount()) return this.removeStackFromSlot(i);
