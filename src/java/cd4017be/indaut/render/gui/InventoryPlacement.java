...
public class InventoryPlacement {
	...
	public ItemStack doPlacement(World world, EntityPlayer player, BlockPos pos, int i, ItemStack stack) {
		...
		ItemStack lItem = player.inventory.mainInventory.get(ci);
		...
		if (obj != null) {
			...
			flag = /*(event.useBlock != Event.Result.DENY && */state.getBlock().onBlockActivated(world, pos, state, player, EnumHand.MAIN_HAND, obj.sideHit, X, Y, Z) /*) || event.useItem == Event.Result.DENY */;
		}
		if (!flag) {
			...
			//if (event.useBlock != Event.Result.DENY && event.useItem != Event.Result.DENY) 
				stack.getItem().onItemUse(player, world, pos, EnumHand.MAIN_HAND, dir.getOpposite(), X, Y, Z);
		}
		stack = player.getHeldItemMainhand();
		if (stack != null && stack.getCount() <= 0) stack = null;
