...
import cd4017be.indaut.item.ItemMatterInterface.GuiData;
...
public class GuiItemMatterInterface extends GuiMachine {
	...
	class ItemList extends GuiComp {
		...
		@Override
		public void draw() {
			list = MatterOrbItemHandler.getAllItems(data.inv.inv.mainInventory.get(data.inv.tool));
			...
			for (int i = 0; i < 12 && i < list.length - scr; i++)
				fontRendererObj.drawString("" + list[i + scr].getCount(), px + 8 + (i % 2) * w2, py + i / 2 * 8, 0x7fffff);
		}
	...
	class SlotSel extends GuiComp {
		...
		public void drawOverlay(int mx, int my) {
			...
			for (int i = cont.invPlayerS; i < cont.invPlayerE; i++) {
				...
				if (slot.getSlotIndex() == src) drawTexturedModalRect(slot.xPos + guiLeft, slot.yPos + guiTop, 194, 18, 16, 16);
				else if (slot.getSlotIndex() == target) drawTexturedModalRect(slot.xPos + guiLeft, slot.yPos + guiTop, 210, 18, 16, 16);
			}
		}
