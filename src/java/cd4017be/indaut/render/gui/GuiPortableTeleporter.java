...
public class GuiPortableTeleporter extends GuiMachine {
	...
	public void updateScreen() {
		...
		if (item == null || !item.hasTagCompound()) {
			...
			int x = MathHelper.floor(inv.player.posX);
			int y = MathHelper.floor(inv.player.posY) - 1;
			int z = MathHelper.floor(inv.player.posZ);
			int d = inv.player.world.provider.getDimension();
	...
	protected void setDisplVar(int id, Object obj, boolean send) {
		...
		if(id == 24){scroll = Math.max(0, Math.round((Float)obj * (float)(list.length + 1 - size))); return;}
			...
			Entry e = list[sel];
			if (id == 26) {
				e.x = MathHelper.floor(inv.player.posX);
				e.y = MathHelper.floor(inv.player.posY) - 1;
				e.z = MathHelper.floor(inv.player.posZ);
				e.dim = inv.player.world.provider.getDimension();
	...
	private static class Entry {
		...
		public void draw(GuiPortableTeleporter gui, int x, int y) {
			...
			gui.drawStringCentered(TooltipUtil.formatNumber(dist, 3, 0) + "m", x + 133, y, dist <= gui.maxDist ? dist <= gui.maxDist / 2D ? 0x40ff40 : 0xffff40 : 0xff4040);
		}
