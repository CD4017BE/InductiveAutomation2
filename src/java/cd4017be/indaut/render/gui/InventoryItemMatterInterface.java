...
public class InventoryItemMatterInterface implements IInventory
{
	...
	public void onCommand(PacketBuffer dis) throws IOException
	{
		...
		if (cmd == 0) { //flip Stack
			...
			if (item == null) return;
			if (n > 0 && item.getCount() > n) item.setCount(n);
			else n = item.getCount();
	...
	public ItemStack decrStackSize(int i, int n) 
	{
		...
		ItemStack item = inventory[i].getCount() <= n ? this.removeStackFromSlot(i) : inventory[i].splitStack(n);
	...
	public void setInventorySlotContents(int i, ItemStack item) 
	{
		if (i == 2) {
			inventory[2] = item;
			int n = (inventory[4] != null ? inventory[4].getCount() : 0) - (inventory[2] != null ? inventory[2].getCount() : 0);
	...
	private void updateOutput()
	{
		...
		if (inventory[2] != null && inventory[2].getCount() > 64) inventory[2].setCount(64);
		inventory[4] = inventory[2] != null ? inventory[2].copy() : null;
	}
